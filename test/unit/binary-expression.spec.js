...
test.group('BinaryExpression Expression', (group) => {
  ...
  test('convert with member expression', (assert) => {
    const statement = `user.username === 'virk'`
    this.exp.parse(esprima.parse(statement).body[0].expression)
    assert.equal(this.exp.toStatement(), `this.context.accessChild(this.context.resolve('user'), ['username']) === 'virk'`)
  })
  ...
  test('convert deeply nested right hand side wrapped binary expression', (assert) => {
    const statement = '4 * (4 + 4)'
    this.exp.parse(esprima.parse(statement).body[0].expression)
    assert.equal(this.exp.toStatement(), '4 * (4 + 4)')
  })
})
