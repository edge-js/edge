...
test.group('Buffer', (group) => {
  ...
  test('add a block to isolate the scope', (assert) => {
    const buffer = new Buffer(true)
    buffer.startIsolation()
    const output = buffer.getLines()
  ...
  test('all new lines should be added to isolate callback', (assert) => {
    const buffer = new Buffer(true)
    buffer.startIsolation()
    buffer.writeLine('foo')
    const output = buffer.getLines()
  ...
  test('end isolate callback', (assert) => {
    const buffer = new Buffer(true)
    buffer.startIsolation()
    buffer.writeLine('foo')
    buffer.endIsolation()
  ...
  test('pass values given to endIsolation to the newContext', (assert) => {
    const buffer = new Buffer(true)
    buffer.startIsolation()
    buffer.writeLine('foo')
    buffer.endIsolation(`'hello'`)
    const output = buffer.getLines()
    assert.equal(output, dedent`
    return (function templateFn () {
      let out = new String()
